...
Begin {
	...
	$CreateMsg = new-object byte[] $DummySize; (new-object Random).NextBytes($CreateMsg); [IO.File]::WriteAllBytes("$($Source)\test.txt", $CreateMsg)

    Try {
        $TotalSize = (Get-ChildItem -LiteralPath $Source\Test.txt -ErrorAction Stop).Length
    }
    Catch {
    ...
    {   $Server = $ServerPath.Split("\")[2]
        ...
        Try {
            ...
            $WriteTest = Measure-Command { 
                Copy-Item $Source\Test.txt $Target -ErrorAction Stop | Out-Null
            }
            
            Write-Verbose "$(Get-Date): Read Test..."
            $ReadTest = Measure-Command {
                Copy-Item $Target\Test.txt $Source\TestRead.txt -ErrorAction Stop | Out-Null
            }
            $Status = "OK"
