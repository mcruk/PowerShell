...
Function Clear-UserCacheFiles {
    ...
    ForEach($localUser in (Get-ChildItem -LiteralPath 'C:\users').Name)
    {
...
Function Clear-FirefoxCacheFiles {
    ...
    if((Test-Path "C:\users\$user\AppData\Local\Mozilla\Firefox\Profiles"))
    {
        ...
        $firefoxAppDataPath = (Get-ChildItem -LiteralPath "C:\users\$user\AppData\Local\Mozilla\Firefox\Profiles" | Where-Object { $_.Name -match 'Default' }[0]).FullName 
        ForEach($cachePath in $possibleCachePaths)
        {
...
Function Clear-WaterfoxCacheFiles { 
    ...
    {
        $possibleCachePaths = @('cache','cache2\entries','thumbnails','cookies.sqlite','webappsstore.sqlite','chromeappstore.sqlite')
        $waterfoxAppDataPath = (Get-ChildItem -LiteralPath "C:\users\$user\AppData\Local\Waterfox\Profiles" | Where-Object { $_.Name -match 'Default' }[0]).FullName
        ForEach($cachePath in $possibleCachePaths)
        {
...
Function Clear-TeamsCacheFiles { 
    ...
    if((Test-Path "C:\users\$user\AppData\Roaming\Microsoft\Teams"))
    {
        ...
        $teamsAppDataPath = (Get-ChildItem -LiteralPath "C:\users\$user\AppData\Roaming\Microsoft\Teams" | Where-Object { $_.Name -match 'Default' }[0]).FullName
        ForEach($cachePath in $possibleCachePaths)
        {
