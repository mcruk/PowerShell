...
Title="SistemasWin | MiShell Deploy | $identity" Height="550" Width="1100">
	<Grid x:Name="Grid" Margin="0" ShowGridLines="False" >
		...
		<StackPanel Margin="2,4,2,0">
			...
			<CheckBox x:Name="CheckBoxParallel" Content="Run In Parallel" ToolTip="AllAtOnce=ON, OneAtTime=OFF" Margin="0,2,0,4"/>
			</StackPanel>
		</Grid>
...
$uihash.ButtonComputers.Add_Click( {
		...
		$OpenFileDialog.initialDirectory = "$($uihash.PSScriptroot)\ComputerList"
				...
				$False {
					...
					$scriptBlock = {
						...
						do {
							...
							$computernameArray = $uihash.objcomputers
							...
							if ([bool]$computername -And $uihash.CheckBoxParallel.isChecked -eq $true) {
								...
								$IsJobRunning = $global:Jobs | Get-Job | Select-Object -Property State, Name | Out-String
								#@{State=Running; Name=203-01} @{State=Running; Name=203-02} @{State=Running; Name=203-03}
								
								
								<# $ggg = Get-Job -State "Running"
								write-host $ggg
								$bbb =  Get-Job | Select-Object -Property State, Name
								write-host $bbb
								$sss = $global:Jobs | Get-Job | Select-Object -Property State, Name
								write-host $sss #>
								
								if($global:Jobs | Get-Job | Select-Object -Property State, Name | Out-String){
									write-host "yay"
								}
																
								While($IsJobRunning.contains("Running") -eq $true){
									
									Start-Sleep 10
									
									foreach($job in $global:Jobs){
										
										$test = $job | Get-Job | Select-Object -Property State, Name 
										
										if(!$CompletedJobs.Contains($test.Name)){

											if($test.State -eq "Completed"){
												
												Out-TextBlock -Source $scriptbasename -Message "$($test.Name) Completed" -Messagetype 'OK' -logfile 'ps1command.log'
												$CompletedJobs += $test.Name 
												

											}

											if($test.State -eq "Failed"){
												
												Out-TextBlock -ComputerName $computername -Source $scriptbasename -Message "$($test.Name) FAILED" -Messagetype 'Error' -logfile 'ps1command.log'
												$CompletedJobs += $test.Name 
							...
							if ([bool]$computername -And $uihash.CheckBoxParallel.isChecked -eq $false) {
								switch ($uihash.extension) {
									"bat" {
